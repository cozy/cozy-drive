...
export class SelectServer extends Component {
  ...
  render() {
    ...
    const { t, previousStep, breakpoints: { isMobile } } = this.props
      ...
      >
        <div className={styles['wizard-wrapper']}>
          <header className={styles['wizard-header']}>
            ...
            />
            <MainTitle tag="h1" className={classNames(styles['wizard-title'], 'u-mb-1', 'u-mt-0')}>
          ...
          <div className={styles['wizard-main']}>
            ...
            <div className={styles['wizard-dualfield']}>
              ...
              <select className={classNames(
                styles['wizard-select'], {
                  [styles['wizard-select--medium']] : isMobile
                }
              )}>
                <option value=".mycozy.cloud">{t('mobile.onboarding.server_selection.domain_cozy')}</option>
                <option value="custom">{t('mobile.onboarding.server_selection.domain_custom')}</option>
              </select>
            </div>
            {!error && (
              <ReactMarkdown
                className={classNames(styles['wizard-notice'], styles['wizard-notice--lost'])}
                source={t('mobile.onboarding.server_selection.lostpwd')}
              />
            )}
            {error && (
              <ReactMarkdown
                className={classNames(styles['wizard-notice'], styles['wizard-notice--error'])}
                source={t(error)}
              />
            )}
          </div>
