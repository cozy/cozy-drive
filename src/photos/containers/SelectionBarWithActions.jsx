      ...
      action: selected =>
        confirm(
          <DeleteConfirm
            ...
            related={ownProps.related}
          />,
          () => dispatch(deletePhotos(selected))
        ),
      displayCondition: () =>
        ownProps.router.location.pathname.startsWith('/photos')
    },
    'album-remove': {
      action: selected =>
        dispatch(removeFromAlbum(ownProps.album, selected))
          .then(() => {
            Alerter.success('Albums.remove_photos.success', {
              album_name: ownProps.album.name
            })
      ...
      displayCondition: () =>
        ownProps.album !== undefined && ownProps.readOnly === false
    }
  }
})

export default translate()(
  withRouter(connect(null, mapDispatchToProps)(SelectionBar))
)
