...
export default class ShareAutocomplete extends Component {
  ...
  onSuggestionsFetchRequested = ({ value }) => {
    this.setState(state => ({...state, suggestions: this.computeSuggestions(value)}))
  }
      ...
      <div className={styles.recipientsContainer}>
        {this.props.recipients.map(recipient =>
          <div className={styles.recipientChip}>
            <span>{recipient.id ? getPrimaryEmail(recipient) : recipient.email}</span>
            <button className={styles.removeRecipient} onClick={() => this.onRemove(recipient)} />
          </div>
        )}
        <div><input {...props} onKeyPress={this.onKeyPress} /></div>
      </div>
    )
  }
  ...
  render () {
    ...
    return (
      <Autosuggest
        ref={(self) => { this.input = self ? self.input : null }}
        theme={styles}
