...
const SettingCategory = ({ title, elements }) => (
  <div className={styles['settings__category']}>
    ...
    {elements.map(element => (
      element.display === false ? '' : <div>
        {element.title && <h4 className={styles['settings__subcategory-title']}>{element.title}</h4>}
        {element.description && <p className={styles['settings__subcategory__label']}>{element.description}</p>}
        ...
        {element.type === ELEMENT_CHECKBOX &&
          <div className={styles['settings__subcategory']}>
            <label for={element.id} className={styles['settings__subcategory__label']}>{element.label}</label>
            <label for={element.id} className={styles['settings__subcategory__item']}>
              <Toggle id={element.id} checked={element.checked} onToggle={element.onChange} />
            </label>
          </div>
        }
        {element.type === ELEMENT_BUTTON &&
          <Button busy={element.busy} theme={element.theme} onClick={element.onClick} className={styles['settings__button']}>{element.text}</Button>
        }
      </div>
    ))}
  </div>
